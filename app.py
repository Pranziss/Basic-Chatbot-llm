...
from voice import synthesize, play_voice
...
Nova:"""
    ...
    try:
        ...
        try:
            filename = asyncio.run(synthesize(reply, mood))
            if filename:
                play_voice(filename)
            else:
                print("[VOICE ERROR] No audio file returned.")
        except Exception as e:
            print("[VOICE SYNTH ERROR]", e)
...
if __name__ == "__main__":
    print("🚀 Nova is booting up on http://127.0.0.1:5000/")
    app.run(debug=True)